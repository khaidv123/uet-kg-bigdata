...
### Ivy cache permission error

```
java.io.FileNotFoundException: /home/spark/.ivy2/cache/...
```

**Fix:** Thêm `--conf spark.jars.ivy=/tmp/ivy2` và `mkdir -p /tmp/ivy2/cache /tmp/ivy2/jars` trước khi submit.

### Port conflict (7077, 8080)

Nếu đã có Spark cluster (phase2) đang chạy, các port mặc định sẽ bị chiếm. File `docker-compose.graphx.yml` đã dùng port khác: `7078`, `8092`, `8093`.

### GraphFrames package not found

Nếu Maven repository không accessible từ trong container, tải jar thủ công:

```bash
# Trên host
wget https://repos.spark-packages.org/graphframes/graphframes/0.8.3-spark3.5-s_2.12/graphframes-0.8.3-spark3.5-s_2.12.jar

# Mount và dùng --jars thay vì --packages
docker compose run --rm \
  -v ./graphframes-0.8.3-spark3.5-s_2.12.jar:/opt/spark/extra-jars/graphframes.jar \
  spark-submit-graphx bash -c '
  /opt/spark/bin/spark-submit \
    --jars /opt/spark/extra-jars/graphframes.jar \
    ...
'
```

### Docker stdout buffering

Docker compose run buffer stdout. Để xem output realtime:

```bash
# Dùng --no-TTY và pipe qua unbuffer
docker compose run -T --rm spark-submit-graphx bash -c '...' 2>&1 | tee output.log
```
